        ...
        for l,(num_w,num_b) in enumerate(zip(w_size_list,b_size_list)):
            ...
            if NN_type_list[l]=='cnn':
                for n in range(NN_size_list[l+1]):
                    sum_grad=torch.tensor(tmp_w[:(kernel_size_list[l][0]*kernel_size_list[l][1])*NN_size_list[l]]).clone().detach().sum().item()
                    dist_grad_w_node_list[l][n]=tmp_w[:(kernel_size_list[l][0]*kernel_size_list[l][1])*NN_size_list[l]]
                    print('\r {} {} {}'.format(l,n,t),end='')
                    sum_grad_w_node_list[l][n][t]=sum_grad
                    avg_grad_w_node_list[l][n][t]=sum_grad/float(NN_size_list[l+1])
                    tmp_w =tmp_w[(kernel_size_list[l][0]*kernel_size_list[l][1]**2)*NN_size_list[l]:]

            elif NN_type_list[l]=='fc':
                for n in range(NN_size_list[l+1]):
                    sum_grad=torch.tensor(tmp_w[:NN_size_list[l]]).sum().clone().detach().item()
